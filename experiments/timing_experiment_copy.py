    ...
    for num_items in num_items_list:
        ...
        for k in k_list:
            ...
            if args.run_CCG_cuts:
                ...
                #                                                     raise_gurobi_time_limit=False,
                #                                                     )
                queries, objval, _ = solve_scenario_decomposition(items, k, rs, valid_responses, args.u0_type,
                                                                    cut_1=True,
                                                                    cut_2=True,
                                                                    max_iter=1000000,
                                                                    problem_type='maximin',
...
def main():
    ...
    parser.add_argument(
        "--run-MILP",
        action="store_true",
        help="if set, use MILP",
        default=False,
    )
    parser.add_argument(
        "--run-MILP-cuts",
        action="store_true",
        help="if set, use MILP with symmetry breaking cuts",
        default=False,
    )
    parser.add_argument(
        "--run-CCG",
    ...
    parser.add_argument(
        "--run-CCG-cuts",
        action="store_true",
        help="if set, use CCG with symmetry breaking cuts",
        default=False,
    )
    parser.add_argument(
        "--run-heuristic",
        action="store_true",
        help="if set, use heuristic",
        default=False,
    )
    ...
    if args.DEBUG:
        ...
        arg_str += " --time-limit 60"
        arg_str += " --num-items 5"
        arg_str += " --run-heuristic"
        arg_str += " --run-CCG-cuts"
        arg_str += " --u0-type box"
