...
output_file = '/Users/duncan/research/ActivePreferenceLearning/RobustActivePreferenceLearning_output/gold/{}/static_experiment_20200107_134538.csv'.format(version_str)
...
plt.suptitle("static heuristic compared to 50 random query sample", fontsize=12)
axs[0, 0].legend()
...
def plot_runtimes(df, ax, k_list):
    ...
    # plot median, min, and max val of random
    time_list = [df[(df['method'] == 'random') & (df['K'] == k)]['objective_eval_time'].median() for k in k_list]
    val_list = np.cumsum(time_list)
    ax.plot(k_list, val_list, label='RAND (obj. eval)', **rand_style_med)

    # plot median, min, and max val of random
    time_list = [df[(df['method'] == 'random') & (df['K'] == k)]['objective_eval_time'].min() for k in k_list]
    val_list = np.cumsum(time_list)
    ax.plot(k_list, val_list, label='RAND (obj. eval)', **rand_style_min)

    # plot median, min, and max val of random
    time_list = [df[(df['method'] == 'random') & (df['K'] == k)]['objective_eval_time'].max() for k in k_list]
    val_list = np.cumsum(time_list)
...
plt.suptitle("static heuristic compared to 50 random query samples", fontsize=12)
axs[0, 0].legend()
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
