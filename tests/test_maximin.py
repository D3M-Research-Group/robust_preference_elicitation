# unit tests for the robust rec-learning problem (maximin objective), without agent inconsistencies.
...
class BaseTestClass:
    ...
    class TestStaticElicitation(unittest.TestCase):
        ...
        def evaluate_problem(self, problem_num, elicitation_func):
            ...
            """
            if problem_num == 1:
                items, K, optimal_objval, valid_responses = self.problem_1()
            elif problem_num == 2:
                items, K, optimal_objval, valid_responses = self.problem_2()
            elif problem_num == 3:
                items, K, optimal_objval, valid_responses = self.problem_3()
            else:
                raise Exception("invalid problem number")

            # run the elicitation method
            queries, obj_val = elicitation_func(items, K, valid_responses)

            # the method should return K queries
            self.assertEqual(len(queries), K)

            # the method should achieve the optimal objective value
            self.assertAlmostEqual(obj_val, optimal_objval, delta=1e-3)
        ...
        def problem_1(self):
            ...
            optimal_objval = -0.266

            return items, K, optimal_objval, valid_responses
        ...
        def problem_2(self):
            ...
            optimal_objval = -1.4474631069858173
        ...
        def problem_3(self):
            ...
            optimal_objval = -13.369138793131665

            return items, K, optimal_objval, valid_responses



class TestMIP(BaseTestClass.TestStaticElicitation):
    """check that the MIP formulation without cuts solves the test problems"""

    def elicitation_func(self, items, K, valid_responses):
