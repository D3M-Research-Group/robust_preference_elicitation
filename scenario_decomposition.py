...
def feasibility_subproblem(
    ...
    if gamma_inconsistencies > 0:
        # create error variables for each item
        eps_vars = m.addVars(num_items, K, lb=0.0, ub=GRB.INFINITY)
        for i_item in range(num_items):
            # for each item, sum of errors are bounded by \Gamma
            m.addConstr(quicksum(eps_vars[i_item, k] for k in range(K)) <= gamma_inconsistencies)
    else:
        eps_vars = np.zeros((num_items, K))
    ...
    for i in range(max_iter):
        ...
        if UB - LB <= delta:
            if UB - LB < - 1e-3:
                ...
                print(f"Exception: UB < LB : UB = {UB}, LB = {LB}")
            break
