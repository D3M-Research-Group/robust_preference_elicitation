...
class Agent(object):
    ...
    def calculate_alpha(self, sigma, num_features, item_sphere_size=10.0, seed=0):
        ...
        for i in range(num):
            ...
            z = items[0].features - items[1].features
            d = np.dot(z, self.u_true) + error
            d_true = np.dot(z, self.u_true)
            if d * d_true < 0:
                num_error += 1

        return 1 - num_error / num
...
def generate_items(
    num_features, num_items, item_sphere_size=1.0, seed=None, positive=False
):
    rs = np.random.RandomState(seed)
    return [
        Item.random(
